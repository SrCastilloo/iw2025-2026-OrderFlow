...
import com.vaadin.flow.server.VaadinSession; // Importante para persistir el Locale
...
public class RegistroView extends VerticalLayout {
    ...
    // Constante para guardar el locale en la sesión
    ...
    @Autowired
    public RegistroView(RegistrarCliente registrarCliente, SimpleI18NProvider i18nProvider) {
        this.registrarCliente = registrarCliente;
        this.i18nProvider = i18nProvider;

        // =========================================================
        // 1. LÓGICA DE RECUPERACIÓN DE IDIOMA AL INICIO
        // =========================================================
        ...
        // =========================================================
        // 2. CREACIÓN Y LÓGICA DEL SELECTOR DE IDIOMA
        // =========================================================
        ...
        languageSelect.addValueChangeListener(event -> {
            ...
            if (newLocale != null && !newLocale.equals(UI.getCurrent().getLocale())) {
                ...
                // Actualizar UI y recargar la página para que se apliquen las traducciones
        ...
        topRightContainer.getStyle().set("padding-right", "30px");
        ...
        H1 title = new H1(tr("view.register.title")); // 🚨 TRADUCIDO
        title.getStyle().set("margin", "0")
                .set("font-size", "clamp(28px, 3vw, 40px)")
                .set("letter-spacing", "-0.02em")
                .set("color", "hsl(14, 90%, 24%)");

        Paragraph subtitle = new Paragraph(tr("hero.subtitle.register")); // 🚨 TRADUCIDO
        ...
        TextField nombre = new TextField(tr("field.name")); // 🚨 TRADUCIDO
        TextField apellidos = new TextField(tr("field.surname")); // 🚨 TRADUCIDO
        EmailField email = new EmailField(tr("field.email")); // 🚨 TRADUCIDO
        PasswordField password = new PasswordField(tr("field.password")); // 🚨 TRADUCIDO
        TextField telefono = new TextField(tr("field.phone")); // 🚨 TRADUCIDO
        TextField calle = new TextField(tr("field.street")); // 🚨 TRADUCIDO
        TextField numero = new TextField(tr("field.number")); // 🚨 TRADUCIDO
        TextField ciudad = new TextField(tr("field.city")); // 🚨 TRADUCIDO
        TextField provincia = new TextField(tr("field.province")); // 🚨 TRADUCIDO
        TextField codigoPostal = new TextField(tr("field.zip")); // 🚨 TRADUCIDO
        ComboBox<String> pais = new ComboBox<>(tr("field.country")); // 🚨 TRADUCIDO

        // Mapeo de países a sus nombres en español, manteniendo las claves en español para el ComboBox
        // En una app real, esto se manejaría con un Enum y I18N
        pais.setItems("España","Francia","Portugal","Alemania","Italia");
        pais.setAllowCustomValue(false);

        Button registrar = new Button(tr("button.register")); // 🚨 TRADUCIDO
        ...
        email.setErrorMessage(tr("validation.email_invalid"));
        email.setValue("ejemplo@gmail.com");
        email.setClearButtonVisible(true);

        nombre.setPlaceholder(tr("field.name_placeholder")); // 🚨 TRADUCIDO
        apellidos.setPlaceholder(tr("field.surname_placeholder")); // 🚨 TRADUCIDO
        email.setPlaceholder("tucorreo@dominio.com");
        password.setPlaceholder(tr("field.password_placeholder_register")); // 🚨 TRADUCIDO (Mín. 8 caracteres)
        telefono.setPlaceholder(tr("field.phone_placeholder")); // 🚨 TRADUCIDO (+34 6XX XX XX XX)
        calle.setPlaceholder(tr("field.street_placeholder")); // 🚨 TRADUCIDO
        numero.setPlaceholder(tr("field.number_placeholder")); // 🚨 TRADUCIDO
        ciudad.setPlaceholder(tr("field.city_placeholder")); // 🚨 TRADUCIDO
        provincia.setPlaceholder(tr("field.province_placeholder")); // 🚨 TRADUCIDO
        codigoPostal.setPlaceholder(tr("field.zip_placeholder")); // 🚨 TRADUCIDO
        pais.setPlaceholder(tr("field.country_placeholder")); // 🚨 TRADUCIDO
        ...
        Paragraph passLabel = new Paragraph(tr("validation.password_strength")); // 🚨 TRADUCIDO
        ...
        password.addValueChangeListener(e -> {
            ...
            if (score < 40) {
                ...
                password.setErrorMessage(tr("validation.password_weak")); // 🚨 TRADUCIDO
        ...
        telefono.addValueChangeListener(e -> {
            ...
            if (!ok) telefono.setErrorMessage(tr("validation.phone_format")); // 🚨 TRADUCIDO
            okTel.setVisible(ok);
        });
        ...
        Runnable validateCP = () -> {
            ...
            if (!ok) codigoPostal.setErrorMessage(tr("validation.zip_error", pais.getValue()==null ? tr("field.country_generic") : pais.getValue())); // 🚨 TRADUCIDO
            okCP.setVisible(ok);
        };
        ...
        // Footer legal
        Paragraph legal = new Paragraph(tr("text.accept_terms")); // 🚨 TRADUCIDO
        legal.getStyle().set("margin", "6px 0 0 0").set("font-size", "12px").set("opacity", ".7");


        // Botón tipo enlace: “¿Ya tienes una cuenta?”
        Button loginBtn = new Button(tr("link.go_login")); // 🚨 TRADUCIDO
        ...
        // 2. Añadir el contenedor del selector al inicio
        ...
        registrarBtn.addClickListener(e -> {
            ...
            if (!allOk) {
                Notification n = Notification.show(tr("notification.review_fields")); // 🚨 TRADUCIDO
            ...
            // Guardamos la dirección concatenada
            ...
            registrarBtn.setText(tr("button.processing")); // 🚨 TRADUCIDO
            ...
            try {
                ...
                getUI().ifPresent(ui -> {
                    ...
                    new Thread(() -> {
                        ...
                        ui.access(() -> {
                            Notification n = Notification.show(tr("notification.register_success", finalCliente.getNombre())); // 🚨 TRADUCIDO con nombre
                            ...
                            // Navegar al login
                            getUI().ifPresent(u -> u.navigate(LoginView.class));
                        });
                    }).start();
                ...
                n.addThemeVariants(NotificationVariant.LUMO_ERROR);
                n.setPosition(Notification.Position.MIDDLE);
                topProgress.removeClassName("loading");
                ...
                // Devolvemos el botón a su estado original si falla o si la navegación no ocurre inmediatamente
                registrarBtn.setEnabled(true);
                registrarBtn.setText(tr("button.register")); // 🚨 TRADUCIDO
                registrarBtn.setIcon(VaadinIcon.FLASH.create());
            }
