...
public class InsertarProductoCarrito {
    ...
    public InsertarProductoCarrito(CarritoRepository carritoRepository,Detalle_CarritoRepository detalleCarritoRepository,
    ...
    public Carrito meterProductoCarrito(Long clienteid, Long productoid, Integer cantidad )
    {
        ...
        Detalle_Carrito detalle = detalleCarritoRepository.findByCarrito_IdAndProducto_Id(c.getId(), productoid).orElse(null);
        ...
        double nuevoTotal = c.getDetalles().stream().mapToDouble(Detalle_Carrito::getSubtotal).sum();
